...
sr1([do,not|X],[dont|Y],X,Y).
sr1([can,not|X],[cant|Y],X,Y).
sr1([cannot|X],[cant|Y],X,Y).
sr1([will,not|X],[wont|Y],X,Y).
sr1([dreamed|X],[dreamt|Y],X,Y).
sr1([dreams|X],[dream|Y],X,Y).
sr1([how|X],[what|Y],X,Y).
sr1([when|X],[what|Y],X,Y).
sr1([alike|X],[dit|Y],X,Y).
sr1([same|X],[dit|Y],X,Y).
sr1([certainly|X],[yes|Y],X,Y).
sr1([maybe|X],[perhaps|Y],X,Y).
sr1([deutsch|X],[xfremd|Y],X,Y).
sr1([francais|X],[xfremd|Y],X,Y).
sr1([espanol|X],[xfremd|Y],X,Y).
sr1([machine|X],[computer|Y],X,Y).
sr1([machines|X],[computer|Y],X,Y).
sr1([computers|X],[computer|Y],X,Y).
sr1([am|X],[are|Y],X,Y).
sr1([your|X],[my|Y],X,Y).
sr1([were|X],[was|Y],X,Y).
sr1([me|X],[you|Y],X,Y).
sr1([you,are|X],[im|Y],X,Y).      % im = i'm = i am
sr1([i,am|X],[youre|Y],X,Y).      % youre = you're = you are =\= your
sr1([myself|X],[yourself|Y],X,Y).
sr1([yourself|X],[myself|Y],X,Y).
sr1([mom|X],[mother|Y],X,Y).
sr1([dad|X],[father|Y],X,Y).
sr1([i|X],[you|Y],X,Y).
sr1([you|X],[i|Y],X,Y).
sr1([my|X],[your|Y],X,Y).
sr1([everybody|X],[everyone|Y],X,Y).
sr1([nobody|X],[everyone|Y],X,Y).
...
char_type_sw(46,period) :- !.
char_type_sw(X,alphanumeric) :- X >= 65, X =< 90, !.
char_type_sw(X,alphanumeric) :- X >= 97, X =< 123, !.
char_type_sw(X,alphanumeric) :- X >= 48, X =< 57, !.
char_type_sw(X,whitespace) :- X =< 32, !.
char_type_sw(X,punctuation) :- X >= 33, X =< 47, !.
char_type_sw(X,punctuation) :- X >= 58, X =< 64, !.
char_type_sw(X,punctuation) :- X >= 91, X =< 96, !.
char_type_sw(X,punctuation) :- X >= 123, X =< 126, !.
char_type_sw(_,special).
...
simplify(List,Result) :- sr1(List,Result,X,Y), !,
			 simplify(X,Y).
